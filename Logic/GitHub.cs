...
public static class GitHub //TODO: Update it with new version of my Github library
{
	...
	/// <summary>
	/// Get list of all versions in the repository
	/// </summary>
	
	 public static async Task<IEnumerable<Release>> GetAllReleasesAsync(string Author, string RepositoryName)
	{
		HttpClientHandler clientHandler = new()
		{
			ServerCertificateCustomValidationCallback = (sender, cert, chain, sslPolicyErrors) => true
		...
		var connection = new Connection(
			...
			new HttpClientAdapter(() => clientHandler));
		var github = new GitHubClient(connection);
		var releases = await github.Repository.Release.GetAll(Author, RepositoryName).ConfigureAwait(false);
		return releases;
	}
